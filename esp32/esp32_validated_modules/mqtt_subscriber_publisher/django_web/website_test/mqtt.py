...
def mqtt_publish(topic: str, payload: dict):
    """
    Função genérica para publicar uma mensagem MQTT.
    :param topic: O tópico onde será feita a publicação.
    :param payload: O payload a ser enviado como dicionário (convertido para JSON).
    :raises: Levanta uma exceção se houver falha na publicação.
    """
    try:
        single(
            topic,
